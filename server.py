...
def disp_individual_product(product_id):
    ...
    reviews_query = db.query('select review.id as review_id, review.rating as rating, date(review.date) as review_date, users.user_name from product inner join review on product.id = review.product_id inner join users on users.id = review.user_id and product.id = %s order by %s %s' % (product_id, sort_method, direction))
...
def render_review():
    ...
    second_cat_query = db.query('prselect name from secondary_cat').namedresult()
...
# def sort_the_category():
#     cat_query = db.query('select name from main_cat where id = %s' % cat_id)
#     show_sub = db.query("select secondary_cat.id as secondary_cat_id, secondary_cat.name as secondary_cat_name, company.id as company_id, company.name as company_name, product.id as product_id, product.name as product_name from main_cat inner join secondary_cat on main_cat.id = secondary_cat.main_cat_id secondary_cat inner join sec_and_com on secondary_cat.id = sec_and_com.secondary_cat_id inner join company on company.id = sec_and_com.company_id inner join product on company.id = product.company_id where secondary_cat.main_cat_id = %s" % sec_cat_id)
#     print show_sub
#     return render_template(
#         '/check.html',
#         cat_id = cat_id,
#         cat_query=cat_query,
#         show_sub = show_sub.namedresult()
#     )

############################################################################################################
select main_cat.id as main_cat_id, main_cat.name as main_cat_name, secondary_cat.id as secondary_cat_id, secondary_cat.name as secondary_cat_name, company.id as company_id, company.name as company_name, product.id as product_id, product.name as product_name
from main_cat
		inner join
	secondary_cat on main_cat.id = secondary_cat.main_cat_id
		inner join
	sec_and_com on secondary_cat.id = sec_and_com.secondary_cat_id
		inner join
	company on company.id = sec_and_com.company_id
		inner join
	product on company.id = product.company_id
		where
	secondary_cat.main_cat_id = 1;
############################################################################################################
